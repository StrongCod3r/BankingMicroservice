...
using Banking.Account.Command.Application.Features.BankAccounts.Commands.UpdateAccountName;
...
namespace Banking.Account.Command.Api.Controllers
{
    ...
    public class BankAccountOperationsAccountController : ControllerBase
    {
        ...
        [HttpPut("UpdateAccountName/{id}", Name = "UpdateAccountName")]
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesDefaultResponseType]
        public async Task<ActionResult<bool>> UpdateAccountName(string id, [FromBody] UpdateAccountNameCommand command)
        {
            command.Id = id;
            return await _mediator.Send(command);
        }
